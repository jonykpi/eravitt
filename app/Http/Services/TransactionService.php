...
class TransactionService
{
    ...
    public function send($wallet, $address, $amount, $is_admin = false, $pendingTransaction = null, $authId = null, $message =null, $tempWithdrawId = null)
    {
        ...
        try {
            ...
            $btc_dlr = converts_currency($doller, $wallet->coin_type,$api_rate);
            ...
            try {
                ...
                if(!empty($tempWithdrawId)) {
                    ...
                    if(!$success) Log::warning('withdraw_id saving failed in temp_withdraws.');
                }
...
//        }
    ...
    {
        $extra_balance = bcsub(bcadd($deposits, $withdrawals,8), $balance,8);
        if ( $extra_balance < 0 ) {
            if ( !empty($suspend) ) {
    ...
    public function sendChainExternal($wallet, $address, $amount){
        ...
        try {
            ...
            DB::beginTransaction();
                ...
                $transaction = WithdrawHistory::create($transactionArray);
                 DB::commit();
            return ['success' => true,'temp'=>$transaction->id, 'message' => __('Withdrawal is now in progress. please wait sometimes until the process complete')];
